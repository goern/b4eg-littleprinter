...
greetings = {"english" => ["Good morning", "Hello", "Good evening"], 
  ...
  post '/handle_push' do
    connection = AMQP.connect(ENV['CLOUDAMQP_URL'])
    channel = AMQP::Channel.new(connection)
    exchange = channel.topic("foursquare")
    channel.queue("4sqvenuecheckins").bind(exchange, :key => "venue.checkin")

    if params[:checkin].nil?
      puts "testing or what?"
    else
      exchange.publish(JSON.parse(params[:checkin]), :routing_key => 'venue.checkin', :content_type => 'application/json')
    end
